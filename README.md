...
- **Сервис авторизации** — обрабатывает регистрацию и вход пользователя, хранит сессии в Redis и информацию о профиле в PostgreSQL. Обеспечивает авторизацию через API Gateway.

- **Сервис пользовательских данных (User_content)** — управляет профилем пользователя, включая личные данные, предпочтения и настройки. Данные хранятся в PostgreSQL, кешируются в Redis.
  
- **Сервис статических географический данных (Static Geo)** — сервис чтения этих данных, выдаёт гео-информацию по запросу других компонентов.
  
- **Сервис динамических географических данных (Dynamic Geo)** — сервис, предоставляющий доступ к актуальной динамической гео-информации, в т.ч. в реальном времени.
  
- **Сервис товаров (Products)** — предоставляет каталог товаров, обрабатывает запросы на получение и изменение карточек продуктов. Данные синхронизируются через Kafka в PostgreSQL и ElasticSearch.

- **Сервис магазинов (Stores)** — управляет информацией о магазинах, их локации и ассортименте. Синхронизируется с поисковыми индексами и аналитикой. Данные хранятся в PostgreSQL.

- **Сервис заказов (Orders)** — обрабатывает оформление, отслеживание и статус заказов. Все операции с заказами записываются в PostgreSQL, данные передаются через Kafka.

- **Сервис отзывов (Reviews)** — отвечает за добавление и отображение отзывов к товарам и магазинам. Данные хранятся в PostgreSQL, обновления поступают другим сервисам через Kafka.

- **Сервис корзины (Cart)** — хранит текущую корзину пользователя в Redis. Обеспечивает быстрый доступ и обновление данных. Kafka используется для синхронизации состояния.

- **Сервис действий пользователя (User actions)** — логирует пользовательскую активность, такую как клики, просмотры, добавления в корзину. Данные могут использоваться для аналитики и рекомендаций.
...
- **Сервис счётчиков магазинов (Stores_counters)** — собирает и агрегирует статистику по магазинам (например, количество заказов, просмотров). Сохраняет данные в ClickHouse.
  
- **Сервис логов (Logs manager)** — получает логи от всех сервисов и отправляет их в хранилище.
  
- **Сервис метрик (Metrics manager)** — агрегирует метрики от сервисов и передаёт их в хранилище.
