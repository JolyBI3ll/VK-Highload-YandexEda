...
|СУБД|	Сущности|
|----|----------|
|PostgreSQL-1 (Транзакции)|	Users, Orders, Reviews, Admin, Card|
|PostgreSQL-2 (Каталог и магазины)|	Stores, Products, Store_inventory, Delivery_zones, Couriers|

Шардируем `Orders` по user_id, `Reviews` по store_id, `Users` по address, `Store_inventory` по store_id, `Products` по category_name, `Couriers` по delivery_zone_name, `Stores` по delievery_zone_name, 

### Репликация


**Стратегия депликации в базе данных PostgreSQL**
