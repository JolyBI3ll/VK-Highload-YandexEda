...
| Дневная аудитория                    | 6 млн человек                |
| Средний размер хранилища пользователя| \~262 КБ                     |
| Количество заказов в день            | 1,98 млн                        |
...
**Казахстан:**
- Использует дата-центр из Новосибирска
...
- Использование SAGA паттерна с оркестратором для организация заказов. Оркестратор на уровне сервиса управления заказами. Например: Оформление заказа = цепочка: подгрузка корзины из сервиса корзины пользователя через апи шлюз -> поиск и назначение ближайшего курьера -> оплата -> уведомление -> завершение. Используется дефолтный режим работы - at-least-once, то есть сообщение будет доставлено по крайней мере один раз и возможны дубли. Для обеспечения выполнения только одной операции на уровне сервиса используются индемпотентые потребители, то есть например, при получении последующих сообщений о необходимости проведения оплаты можно проверить, есть ли запись об оплате в БД с таким же order_id.

- CQRS - раздельный синхронно-асинхронный пайплайн для чтения/записи:
Для чтения и записи в PGSQL рейтинга ресторана и отзыва, для чтения и записи в Elastic индексов продуктов и ресторанов.
...
## 10. Схема проекта
![Схема_проекта.png](assets/project_schema.png)

### Сервисы проекта

- **Микросервис авторизации** — обрабатывает регистрацию и вход пользователя, хранит сессии в Redis и информацию о профиле в PostgreSQL. Обеспечивает авторизацию через API Gateway.

- **Микросервис профиля пользователя** — управляет профилем пользователя, включая личные данные, предпочтения и настройки. Данные хранятся в PostgreSQL, кешируются в Redis.
  
- **Микросервис геолокации и поиска ближайшего курьера** - предоставляет текущее местоположение курьера и находит ближайшего от ресторана курьера.
  
- **Микросервис записи новых продуктов** — обрабатывает запросы на изменение карточек продуктов и добавление новых. Данные синхронизируются через Kafka в PostgreSQL и ElasticSearch.

- **Микросервис записи новых ресторанов** — брабатывает запросы на изменение информации о ресторане и добавление новых. Данные синхронизируются через Kafka в PostgreSQL и ElasticSearch.

- **Микросервис отзывов** — отвечает за добавление и отображение отзывов к магазинам. Данные хранятся в PostgreSQL, обновления поступают другим сервисам через Kafka.

- **Микросервис корзины пользователя** — хранит текущую корзину пользователя в Redis. Обеспечивает быстрый доступ и обновление данных. Kafka используется для синхронизации состояния. Получает команду на создание корзины и возвращает статус.

- **Сервис поиска (Search)** — реализует полнотекстовый поиск по товарам и магазинам через Elasticsearch. Работает совместно с Search Writer, который обновляет индексы на основе данных из PostgreSQL.

- **Микросервис статистики ресторана** — собирает и агрегирует статистику по магазинам (например, количество заказов, просмотров). Сохраняет данные в ClickHouse.
  
- **Микросервис логов** — получает логи от всех сервисов и отправляет их в хранилище.
  
- **Микросервис метрик** — агрегирует метрики от сервисов и передаёт их в хранилище.
  
- **Микросервис истории заказов** - предоставляет список заказов пользователя.
  
- **Микросервис оплаты** - реализует проведение оплаты заказа с помощью внешнего api сервиса оплаты.
  
- **Микросервис управления заказами** - реализует управление жизненным циклом заказа посредством оркестратора, взимодействующий с топиками kafka для обмена сообщениями.
  

## 11. Рассчет ресурсов

Расчёт аппартных ресурсов

Приведем соотношения между характером бизнес-логики и требуемыми ресурсами:

* легкая бизнес-логика: 1 CPU на 1000 RPS и 1 ГБ RAM на 100 RPS;
* средняя бизнес-логика: 1 CPU на 100 RPS и 1 ГБ RAM на 10 RPS;
* тяжелая бизнес-логика: 1 CPU на 10 RPS и 1 ГБ RAM на 10 RPS;

Nginx может поддерживать до 10000 одновременных HTTPS-соединений в секунду на 24 ядрах CPU

### Расчёт базовой нагрузки

Рассчитаем коэффициент для пиковой нагрузки с учетом падения дц в Москве и коэффициентом запаса = 40%:

$K = \left(\frac{L_{\text{макс}}}{L_{\text{ср}}}\right) \left(1 + K_x\right) \left(\frac{N}{N - N_{упав}}\right)$

где:  
$L_{макс} = 1 188.432$ - RPS на самый загруженный ДЦ,

$L_{ср} = 990.36$ - средний по ДЦ RPS,

$K_x = 0.4$ - коэффициент запаса на расширение,

$N = 4$ - общее количество дц,

$N_{упав} = 1$ - количество упавших дц,

**K = 2.24** 

Сервис управлеления заказов имеет RPS такого порядка:
RPS = 23 (создание) + 23 (поход в поисковик ближайшего курьера) + 23 (рейтинг) + 23 (поход в сервис оплаты) = 92

| Сервис                     | Целевая пиковая нагрузка, RPS | Сеть, Гбит/с |
| -------------------------- | ----------------------------- | ------------ |
| Авторизация                | k × 70 = 157                  | 0.00251      |
| Профиль пользователя       | k × 139 = 312                 | 0.00499      |
| Геолокация и поиск курьера | k × (23 + 8250) = 18 532      | 0.2964       |
| Запись новых продуктов     | k × 0.39 = 0.87               | 0.000014     |
| Запись новых ресторанов    | k × 0.14 = 0.31               | 0.00000504   |
| Отзывы                     | k × 3.5 + 173 = 396           | 0.00322      |
| Корзина                    | k × 23 = 52                   | 0.000824     |
| Поиск                      | k × 1040 + 1040 + 208 = 5 126 | 0.0596       |
| Статистика ресторана       | k × 1040 + 23 = 2 382         | 0.0376       |
| История заказов            | k × 21 = 48                   | 0.000752     |
| Оплаты                     | k × 23 = 52                   | 0.000824     |
| Управление заказами        | k × 92 = 207                  | 0.00329      |
| API Gateway                | 22 983                        | 0.4118       |
| Nginx                      | 22 983                        | 0.4118       |
| S3                         | k × 1040 + 1040 + 208 = 5 126 | **2.418**    |

Допущение (Сеть):
- JSON-запросы ~1 KB.
- Видео и изображения — больше: 70-80 KB/s.

Рассчитаем аппартные ресурсы на каждый сервис

| Сервис                     | RPS    | Характер логики | CPU, ядер | RAM, ГБ |
| -------------------------- | ------ | --------------- | --------- | ------- |
| Авторизация                | 157    | лёгкая          | 1         | 2       |
| Профиль пользователя       | 312    | средняя         | 4         | 32      |
| Геолокация и поиск курьера | 18 532 |	лёгкая          |	19        | 186     |
| Запись новых продуктов     | 0.87   | лёгкая          | 1         | 1       |
| Запись новых ресторанов    | 0.31   | лёгкая          | 1         | 1       |
| Отзывы                     | 396    | средняя         | 4         | 40      |
| Корзина                    | 52     | лёгкая          | 1         | 1       |
| Поиск                      | 5 126  | средняя         | 52        | 513     |
| Статистика ресторана       | 2 382  | лёгкая          | 3         | 24      |
| История заказов            | 48     | лёгкая          | 1         | 1       |
| Оплаты                     | 52     | средняя         | 1         | 6       |
| Управление заказами        | 207    | средняя         | 3         | 21      |
| API Gateway                | 22 983 | лёгкая          | 23        | 230     |
| Nginx                      | 22 983 | TLS-handshake   | 105       | 154     |


В итоге на **все ДЦ** получим: 
 - 219 ядер CPU
 - 1 221 ГБ RAM

Теперь рассчитаем аппаратные ресурсы на каждый из наших дц:
**Москва и СПБ**(30% каждый):
- 66 ядер CPU
- 367 ГБ RAM
...
### Рассчет ресурсов по базам данных

Допущения:
- PGSQL: 1 ядро CPU на 100 RPS и 1 ГБ RAM на 10 RPS
- REDIS: 1 ядро CPU на 5000 RPS и 1 ГБ RAM на 1000 RPS
- ClickHouse: 1 ядро CPU на 200 RPS и 1 ГБ RAM на 75 RPS
- ElasticSearch: 1 ядро CPU на 300 RPS и 1 ГБ RAM на 75 RPS

| СУБД          |    RPS    | CPU (ядер) | RAM (ГБ) |
| ------------- | --------- | ---------- | -------- |
| PostgreSQL    |  1 196,18 |    12 |   120 + 68,6 = 188,6   |
| Redis         |  18 741   |     4     |    20    |
| ClickHouse    |  2 382    |     12     |    32   |
| Elasticsearch |  5 126    |      16    |    69   |

### Конфигурация сервисных блоков[^7][^8][^9][^10][^11][^12][^13]

#### Москва и Санкт-Петербург

| Сервисный блок             | Конфигурация сервера                                | CPU cores | Кол-во на 1 ДЦ | Аренда/мес (€) | Покупка (€) | Амортизация/мес (€) |
| -------------------------- | --------------------------------------------------- | --------- | -------------- | -------------- | ----------- | ------------------- | 
| **Kubernetes Worker Node** | 1U AMD EPYC 9124P, 64 cores, 256 GB RAM, 1×NVMe 4TB | 64        | 2              | 2 500          | 20 000      | 555.56              |  
| **PGSQL Node**             | 2U AMD EPYC 7543P, 16 cores, 128 GB RAM, 4×NVMe 8TB | 16        | 2              | 3 500          | 25 000      | 694.44              |   
| **Redis Node**             | 1U AMD EPYC 7302P, 8 cores, 64 GB RAM, 1×NVMe 2TB   | 8         | 2              | 800            | 7 000       | 194.44              |   
| **ClickHouse Node**        | 1U AMD EPYC 7543P, 64 cores, 256 GB RAM, 2×NVMe 4TB | 64        | 1              | 1 200          | 12 000      | 333.33              |   
| **ElasticSearch Node**     | 1U AMD EPYC 7543P, 64 cores, 256 GB RAM, 2×NVMe 4TB | 64        | 1              | 1 200          | 12 000      | 333.33              |   

#### Новосибирск

| Сервисный блок             | Конфигурация сервера                                | CPU cores | Кол-во на 1 ДЦ | Аренда/мес (€) | Покупка (€) | Амортизация/мес (€) |
| -------------------------- | --------------------------------------------------- | --------- | -------------- | -------------- | ----------- | ------------------- |
| **Kubernetes Worker Node** | 1U AMD EPYC 9124P, 32 cores, 256 GB RAM, 1×NVMe 4TB | 32        | 2              | 2 000          | 16 000      | 444.44              |  
| **PGSQL Node**             | 2U AMD EPYC 7543P, 16 cores, 256 GB RAM, 4×NVMe 8TB | 16        | 1              | 3 000          | 22 000      | 611.11              |   
| **Redis Node**             | 1U AMD EPYC 7302P, 4 cores, 32 GB RAM, 1×NVMe 2TB   | 4         | 1              | 600            | 5 000       | 138.89              |   
| **ClickHouse Node**        | 1U AMD EPYC 7543P, 32 cores, 128 GB RAM, 2×NVMe 4TB | 32        | 1              | 1 000          | 10 000      | 277.78              |   
| **ElasticSearch Node**     | 1U AMD EPYC 7543P, 32 cores, 128 GB RAM, 2×NVMe 4TB | 32        | 1              | 1 000          | 10 000      | 277.78              |   

#### Владивосток

| Сервисный блок             | Конфигурация сервера                                | CPU cores | Кол-во на 1 ДЦ | Аренда/мес (€) | Покупка (€) | Амортизация/мес (€) | 
| -------------------------- | --------------------------------------------------- | --------- | -------------- | -------------- | ----------- | ------------------- | 
| **Kubernetes Worker Node** | 1U AMD EPYC 9124P, 16 cores, 128 GB RAM, 1×NVMe 4TB | 16        | 2              | 1 500          | 12 000      | 333.33              |  
| **PGSQL Node**             | 2U AMD EPYC 7543P, 8 cores, 128 GB RAM, 4×NVMe 8TB  | 8         | 1              | 2 500          | 20 000      | 555.56              |  
| **Redis Node**             | 1U AMD EPYC 7302P, 4 cores, 32 GB RAM, 1×NVMe 2TB   | 4         | 1              | 600            | 5 000       | 138.89              | 
| **ClickHouse Node**        | 1U AMD EPYC 7543P, 16 cores, 64 GB RAM, 2×NVMe 4TB  | 16        | 1              | 800            | 8 000       | 222.22              | 
| **ElasticSearch Node**     | 1U AMD EPYC 7543P, 16 cores, 64 GB RAM, 2×NVMe 4TB  | 16        | 1              | 800            | 8 000       | 222.22              | 

#### Общие итоги

| Показатель                  | Значение (€) |
| --------------------------- | ------------ | 
| **Общая стоимость покупки** | 205 000      |   
| **Общая аренда в месяц**    | 24 000       |   
| **Амортизация в месяц**     | 5 694.44     |   
| **Экономия в месяц**        | 18 305.56    |  
| **Окупаемость (в месяцах)** | 11.2         | 
...
[^7]: [AMD Servers | Ryzen Pro and Epyc | IONOS](https://www.ionos.co.uk/servers/amd-servers?__lf=Checkout)
[^8]: [Supermicro 1U AMD Server 1x EPYC 7543 32-Core 2.8GHz 256GB RAM 2x3.84TB NVMe SSD | eBay](https://www.ebay.com/itm/126872363198)
[^9]: [Dell PowerEdge R7615 64C 3.10GHz AMD EPYC | 256GB RAM](https://shop.bytestock.com/dell-poweredge-r7615-1-x-64-core-3-10ghz-amd-epyc-9554p-256gb-2-x-3-84tb-sas-ssd-h755)
[^10]: [Buy AMD EPYC 7302P from £331.86 (Today) – Best Deals on idealo.co.uk](https://www.idealo.co.uk/compare/6750404/amd-epyc-7302p.html)
[^11]: [Buy AMD EPYC 7543 from £2,214.99 (Today) – Best Deals on idealo.co.uk](https://www.idealo.co.uk/compare/205064954/amd-epyc-7543.html)
[^12]: [Dell PowerEdge R6515 1 x 16 Core 3.00GHz AMD EPYC 7302P 64GB 2 x 960GB SATA SSD H740P | shadowcore](https://www.shadowcore.co.uk/shop/dell-poweredge-r6515-1-x-16-core-3-00ghz-amd-epyc-7302p-64gb-2-x-960gb-sata-ssd-h740p-178)
[^13]: [Supermicro 1U AMD Server 1x EPYC 7543 32-Core 2.8GHz 256GB RAM 2x7.68TB NVMe SSD | eBay](https://www.ebay.com/itm/126872510558)
